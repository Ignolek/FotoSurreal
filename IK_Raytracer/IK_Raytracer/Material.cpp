...
Material::Material(vec3 diffuse, vec3 specular, float shine, float fuzziness) : mAmbient(diffuse * 0.05), mDiffuse(diffuse), mSpecular(specular), shininess(shine), fuzz(fuzziness){}
...
Material::Material(vec3 diffuse, vec3 specular, float shine, bool isRefract, float refract) : mAmbient(diffuse * 0.2), mDiffuse(diffuse), mSpecular(specular), shininess(shine), isRefractor(isRefract), refraction(refract){}
